...
import stage_link from '../../../../public/svgs/stage_link.svg';
...
const Preloader = ({
  ...
  const word = ['S', 'T', 'A', 'G', 'E'];
  const Link = ['L', 'I', 'N', 'K'];

  const spans = useRef<any>([]); // Create a ref to store the span elements
const stageSpans = useRef<HTMLSpanElement[]>([]);
const linkSpans = useRef<HTMLSpanElement[]>([]);
  ...
  return (
    <>
      <Wrapper ref={wrapperRef}>
        <Inner>
          <Image ref={imageRef} src={stage_link} alt="import icon" />
          <div>
            {word.map((t, i) => (
              <div
                key={i}
